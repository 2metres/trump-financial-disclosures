<script lang="ts">
  ...
  onMount(async () => {
    ...
    const svgEl = d3.select(container).append('svg').attr('width', size).attr('height', size)
    ...
    node
      ...
      .attr('font-family', (d: any) =>
        d.depth <= 1 ? 'JetBrains Mono, monospace' : 'DM Sans, sans-serif',
      )
    ...
    node
      .on('mouseover', function (event: MouseEvent, d: any) {
        ...
        if (data.entities && data.entities.length > 0) {
          html += `<br/><br/>Top shared holdings:<br/>${data.entities
            .slice(0, 5)
            .map((e: string) => `&middot; ${e}`)
            .join('<br/>')}`
        }
