<script lang="ts">
  ...
  onMount(async () => {
    try {
      ...
      const root = d3.treemap<any>().tile(tile)
        .size([width, height])
      ...
      function render(grp: any, focus: any) {
        const node = grp
          .selectAll('g')
          .data((focus.children || []).concat(focus))
          .join('g')

        // Cursor + click: root node zooms out, children with sub-children zoom in
        node.filter((d: any) => d === focus ? d.parent : d.children)
          .attr('cursor', 'pointer')
        ...
        // Tooltip on non-root nodes
        node.filter((d: any) => d !== focus)
        ...
        node.append('rect')
          ...
          .attr('fill', (d: any) => {
            if (d === focus) return '#1a1a1a'
            const c = getColor(d)
            // Appointee-level nodes (depth 1, no type) get a muted dark fill
            return c === '#525252' && d.children ? '#2a2a2a' : c
          })
          .attr('fill-opacity', (d: any) => d === focus ? 1 : d.children ? 0.85 : 0.75)
          .attr('stroke', '#fff')
          .attr('stroke-width', 1)
        ...
        node.append('text')
          ...
          .attr('font-weight', (d: any) => d === focus ? 'bold' : 500)
          .attr('fill', '#fff')
          .style('pointer-events', 'none')
          .selectAll('tspan')
          .data((d: any) => {
            const label = d === focus ? displayLabel(d) : displayLabel(d)
            const val = d.value ? formatMoney(d.value) : ''
            return val ? [label, val] : [label]
          })
          .join('tspan')
          .attr('x', 4)
          .attr('y', (_: any, i: number, nodes: any) =>
      ...
      function position(grp: any, focus: any) {
        grp.selectAll('g')
          .attr('transform', (d: any) =>
            d === focus ? 'translate(0,-30)' : `translate(${x(d.x0)},${y(d.y0)})`)
          .select('rect')
          .attr('width', (d: any) => d === focus ? width : x(d.x1) - x(d.x0))
          .attr('height', (d: any) => d === focus ? 30 : y(d.y1) - y(d.y0))
      }
...
<div class="relative h-[calc(100vh-200px)]">
  ...
  {:else}
    <!-- Breadcrumb -->
    <div class="absolute top-4 left-6 z-10 font-mono text-[11px] flex items-center gap-1">
